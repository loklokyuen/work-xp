import { collection, addDoc, getDocs, getFirestore, doc, getDoc, query, where, updateDoc, QuerySnapshot } from "firebase/firestore";
...
async function getApplicationsByBusinessId(businessId: string): Promise<Application1[]> {
    const q = query(ApplicationsCollection, where("businessId", "==", businessId));
    const QuerySnapshot = await getDocs(q);
    const applicationsList = QuerySnapshot.docs.map((doc) => {
        return { uid: doc.id, ...doc.data() } as Application1;
    });
...
async function getApplicationByStudentId(studentId: string): Promise<Application1[]> {
...
async function updateApplicationAccepted(appId: string, boolean: boolean): Promise<boolean> {
    try {
