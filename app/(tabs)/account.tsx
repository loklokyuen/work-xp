...
export default function AccountScreen() {
    ...
    return (
        <View style={{ backgroundColor: "#fff", flex: 1, justifyContent: "center", alignItems: "center" }}>
            <Text variant="titleLarge" style={{margin: 20}}>Are you a business or a student user?</Text>
            <Button mode="contained-tonal"
                onPress={() => {
                    setAccountType("Student");
                }}>Student</Button>
            <Text variant="titleLarge" style={{margin: 10}}>OR</Text>
            <Button mode="contained-tonal" 
                onPress={() => {
                    setAccountType("Business");
                }}
            >Business</Button>
            {error ? <Text variant="titleSmall" style={{color: "red"}}>{error}</Text> : null}
            <Button mode="outlined" style={{marginTop: 40}} onPress={handleGuestSignIn}>Sign in as guest</Button>
        </View>
    );
}
