...
const OpportunitiesList: React.FC<{
    ...
    const filteredOpportunities = opportunitiesWithBusinessInfo.filter((opp) => {
        ...
        const matchesSearchQuery = jobRole && jobRole.toLowerCase().includes(searchQuery.toLowerCase());

        const matchesSector = !selectedSector || (business && business.sector === selectedSector);

        const matchesCounty = !selectedCounty || (business && business.county === selectedCounty);
    ...
    return (
        <ScrollView>
            <View style={styles.searchContainer}>
                <SearchBar placeholder="Search for an opportunity..." setSearchQuery={setSearchQuery} isBusinessSearch={false} />
                ...
                <View>
                    {filteredOpportunities.length > 0 ? (
                        filteredOpportunities.map((opp, index) => <OpportunityCard key={index} opp={opp} />)
                    ) : (
